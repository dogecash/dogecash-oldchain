...
*Setup instructions for a gitian build of dogecash using a Debian VM or physical system.*
...
- [Building dogecash](#building-dogecash)
...
Clone the git repositories for dogecash and gitian and then checkout the dogecash version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the dogecash repository
...
To build dogecash (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the dogecash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dogecash/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/dogecash/dogecash
...
For example:
```bash
URL=https://github.com/dogecash/dogecash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit dogecash=${COMMIT} --url dogecash=${URL} ../dogecash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dogecash=${COMMIT} --url dogecash=${URL} ../dogecash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dogecash=${COMMIT} --url dogecash=${URL} ../dogecash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dogecash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dogecash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/dogecash-build.assert
```
...
[eastcoastcrypto/gitian.sigs](https://github.com/dogecashgitian.sigs/) repository, or if that's not possible to create a pull
request.
