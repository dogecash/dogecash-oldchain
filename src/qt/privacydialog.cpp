...
#include "ztrittiumcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTRTT ought to be enough for anybody." - Bill Gates, 2017
    ui->zTRTTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTRTTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTRTT->setEnabled(false);
        ui->pushButtonMintzTRTT->setToolTip(tr("zTRTT is currently disabled due to maintenance."));

        ui->pushButtonSpendzTRTT->setEnabled(false);
        ui->pushButtonSpendzTRTT->setToolTip(tr("zTRTT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTRTTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTRTT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTRTT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTRTT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTRTT in ") +
...
void PrivacyDialog::on_pushButtonSpendzTRTT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTRTT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTRTT
        sendzTRTT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTRTT
    sendzTRTT();
}

void PrivacyDialog::on_pushButtonZTrittiumControl_clicked()
{
    ZTrittiumControlDialog* zTrittiumControl = new ZTrittiumControlDialog(this);
    zTrittiumControl->setModel(walletModel);
    zTrittiumControl->exec();
}

void PrivacyDialog::setZTrittiumControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTrittiumSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTRTT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Trittium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTRTTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTRTT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TRTT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTRTT</b>";
    ...
    // use mints from zTrittium selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTrittiumControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTrittiumControlDialog::GetSelectedMints();
    }
    ...
    // Clear ztrittium selector in case it was used
    ZTrittiumControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTrittium Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Trittium, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTrittium Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTRTT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTRTT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTRTT "));
    ui->labelzTRTTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTRTTSyncStatus->setVisible(fShow);
}
