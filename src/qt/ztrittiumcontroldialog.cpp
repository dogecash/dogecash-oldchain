...
#include "ui_ztrittiumcontroldialog.h"
...
std::list<std::string> ZTrittiumControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTrittiumControlDialog::listMints;

ZTrittiumControlDialog::ZTrittiumControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTrittiumControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTrittiumControlDialog::~ZTrittiumControlDialog()
{
    delete ui;
}

void ZTrittiumControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTrittiumControlDialog::updateList()
{
...
void ZTrittiumControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTrittiumControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTrittium_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTrittiumControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTrittiumControlDialog::GetSelectedMints()
{
...
void ZTrittiumControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
