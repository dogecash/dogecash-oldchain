...
#include "ui_zdogecashcontroldialog.h"
...
std::list<std::string> ZDogeCashControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDogeCashControlDialog::listMints;

ZDogeCashControlDialog::ZDogeCashControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDogeCashControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDogeCashControlDialog::~ZDogeCashControlDialog()
{
    delete ui;
}

void ZDogeCashControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDogeCashControlDialog::updateList()
{
...
void ZDogeCashControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDogeCashControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDogeCash_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDogeCashControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDogeCashControlDialog::GetSelectedMints()
{
...
void ZDogeCashControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
